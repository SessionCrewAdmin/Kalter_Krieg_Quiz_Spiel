...
begin
  if length(v_name) < 1
     or length(v_name) > 80
     or length(v_device) < 8
     or length(v_device) > 200 then
  ...
  -- Current frontend uses roster selection.
  if exists (
    select 1
      from public.classroom_roster
    ...
    return jsonb_build_object('ok', false, 'error', 'name_not_in_roster');
  end if;
  ...
  if v_participant.id is null then
    ...
    update public.classroom_participants
       set device_token = v_device,
           student_token = extensions.gen_random_uuid(),
           last_seen = now()
     where id = v_participant.id
    returning * into v_participant;
